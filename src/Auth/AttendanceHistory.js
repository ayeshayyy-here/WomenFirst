...
const AttendanceHistory = ({ navigation }) => {
  ...
  const generateMonths = () => {
    const months = [];
    const startMonth = new Date(2024, 0);
  ...
  const fetchAttendanceData = async (month, year) => {
    ...
    try {
      const response = await fetch(`https://wwh.punjab.gov.pk/api/attendanceChecklocationservices/${storedUserId}/${month}/${year}`);
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.recordsScroll}>
          {item.records.map((record, index) => (
            <View key={index} style={styles.recordContainer}>
              ...
              >
                <Icon name="map-marker" size={15} color="purple" />
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.userContainer}>
          ...
          <View style={styles.iconnWrapper}>
            <Icon name="user" size={15} color="purple" />
          </View>
        </View>
      </View>
