...
const ProductDisplayBookingScreen = ({ route, navigation }) => {
  ...
  const loadUserProfile = async () => {
    // You can load user profile from AsyncStorage if needed
    // For now, we'll just use the passed user data
    if (user) {
      setFormData(prev => ({
        ...prev,
        full_name: user.name || '',
  ...
  const submitApplication = async () => {
    try {
      ...
      // Product Images
      if (formData.product_image_1 && formData.product_image_1.uri) {
      ...
      if (formData.product_image_2 && formData.product_image_2.uri) {
      ...
      if (formData.product_image_3 && formData.product_image_3.uri) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Success',
        'Product display application submitted successfully!',
        [{ text: 'OK', onPress: () => navigation.goBack() }]
      );
    } catch (error) {
      Alert.alert(
      ...
      <LinearGradient colors={[COLORS.primary, COLORS.primaryDark]} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerCenter}>
            ...
            <Text style={styles.headerSubtitle}>Maryam Nawaz Women Complex</Text>
          </View>
...
export default ProductDisplayBookingScreen;