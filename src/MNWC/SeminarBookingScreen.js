...
const COLORS = {
  ...
  primaryLight: '#0891b2',
  primaryDark: '#0891b2',
  primaryGradient: ['#0891b2', '#0891b2', '#0891b2'],
  primarySoft: '#eff6ff',
  secondary: '#f8f9fa',
  background: '#f4f7fb',
  surface: '#ffffff',
...
const SeminarBookingScreen = ({ route, navigation }) => {
  ...
  // Enforce max participants (70)
  const handleParticipantsChange = (text) => {
    let value = parseInt(text) || '';
    if (value > 70) {
      value = 70;
      Alert.alert('Maximum Limit', 'Maximum 70 participants allowed');
    }
  ...
  const validateForm = () => {
    ...
    if (!formData.seminar_title?.trim()) {
      Alert.alert('Validation Error', 'Title of Seminar / Session is required');
      return false;
    }
    ...
    if (parseInt(formData.num_participants) < 1 || parseInt(formData.num_participants) > 70) {
      Alert.alert('Validation Error', 'Number of participants must be between 1 and 70');
      return false;
    }

    if (!formData.seminar_description?.trim()) {
      Alert.alert('Validation Error', 'Purpose / Brief Description of Seminar is required');
      return false;
    }
    ...
    if (!formData.seminar_date) {
      Alert.alert('Validation Error', 'Seminar Date is required');
      return false;
    }

    // Validate seminar date is in future
    const selectedDate = new Date(formData.seminar_date);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    if (selectedDate <= today) {
      Alert.alert('Validation Error', 'Seminar date must be in the future');
      return false;
    }
  ...
  const submitBooking = async () => {
    try {
      ...
      const textFields = {
        ...
        seminar_date: formData.seminar_date,
      ...
      const response = await fetch(`${API_BASE_URL}/seminar-booking`, {
        method: 'POST',
        headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Success',
        response.message || 'Seminar room booking request submitted successfully!',
  ...
  const renderInfoBar = () => (
    ...
    >
      ...
      <View style={styles.infoItem}>
        ...
        <Text style={styles.infoText}>
          <Text style={styles.infoTextBold}>Max 70 participants</Text>
        </Text>
      </View>
    </LinearGradient>
  );
  ...
  const renderImportantInstructions = () => (
    ...
    >
      ...
      {[
        'The seminar room is to be used strictly for professional, educational, or awareness-raising activities.',
        'The applicant/organizer is responsible for crowd management, discipline, and cleanliness.',
        'All equipment, furniture, and installed systems must be used carefully and responsibly.',
        'Any violation of SOPs may lead to immediate cancellation and restriction of future bookings.',
      ].map((text, index) => (
  ...
  const renderInfoBox = () => (
    ...
    >
      ...
      <Text style={styles.infoBoxText}>
        Bookings exceeding the stated capacity of 70 participants will not be approved.
      </Text>
    </LinearGradient>
  );
  ...
  const renderWarningBox = () => (
    ...
    >
      ...
      <Text style={styles.warningText}>
        MNWC does not provide refreshments. Applicants opting for refreshments must arrange them independently and ensure cleanliness and proper waste disposal after the session.
      </Text>
    </LinearGradient>
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          >
            <View style={styles.sectionHeader}>
              ...
              <Text style={styles.sectionTitle}>Section B: Seminar Details</Text>
            </View>
          </LinearGradient>
          ...
          <View style={styles.sectionContent}>
            ...
            <View style={styles.row}>
              <View style={styles.halfWidth}>
                {renderInput('Title of Seminar / Session', 'seminar_title', { 
                  required: true,
              ...
              <View style={styles.halfWidth}>
                {renderInput('Expected Number of Participants', 'num_participants', { 
                  keyboardType: 'numeric',
                  maxLength: 2,
                  placeholder: 'Max 70 participants',
                  required: true,
                  helper: 'Maximum 70 participants'
                })}
              </View>
            </View>

            {renderInput('Purpose / Brief Description of Seminar', 'seminar_description', { 
              required: true,
              multiline: true,
              numberOfLines: 3,
              placeholder: 'Describe the purpose and objectives of the seminar...'
            })}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.sectionContent}>
            {renderRadioGroup('Will you be using refreshments during the seminar?', 'refreshments', [
              { label: 'Yes', value: 'yes' },
              { label: 'No', value: 'no' },
            ], true)}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.sectionContent}>
            <View style={styles.row}>
              <View style={styles.halfWidth}>
                {renderDatePicker('Seminar Date', 'seminar_date', true)}
              </View>
              <View style={styles.halfWidth}>
          ...
          <View style={styles.sectionContent}>
            ...
            >
              ...
              <Text style={[styles.consentText, formData.consent && styles.consentTextChecked]}>
                I confirm that all information provided is accurate. I agree to comply with all rules, SOPs, and instructions issued by the Maryam Nawaz Women Complex.
              </Text>
            </TouchableOpacity>
          </View>
        </View>
            ...
            {submitting ? (
              ...
              <View style={styles.submitButtonContent}>
                ...
                <Text style={styles.submitButtonText}>
                  Submit Seminar Room Booking Request
                </Text>
              </View>
            )}
          </LinearGradient>
...
export default SeminarBookingScreen;