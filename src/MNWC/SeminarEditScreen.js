...
const COLORS = {
  ...
  primaryLight: '#0e7a94',
  primaryDark: '#065a6f',
  primaryGradient: ['#0891b2', '#07758f', '#055a6f'],
  primarySoft: '#e0f2fe',
  secondary: '#f8f9fa',
  background: '#f4f7fb',
  surface: '#ffffff',
...
const SeminarEditScreen = ({ route, navigation }) => {
  ...
  const fetchBookingData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/seminar-booking/${record_id}`, {
        method: 'GET',
        headers: {
      ...
      console.log('Fetched seminar booking data:', data);
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          seminar_title: booking.seminar_title || '',
          num_participants: booking.num_participants?.toString() || '',
          seminar_description: booking.seminar_description || '',
          refreshments: booking.refreshments || 'no',
          seminar_date: booking.seminar_date || '',
...
const getFullImageUrl = (path) => {
  ...
  const updateBooking = async () => {
    try {
      ...
      console.log('Updating seminar booking with user_id:', user_id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Success',
        'Seminar room booking updated successfully!',
        [{ text: 'OK', onPress: () => navigation.goBack() }]
      );
    } catch (error) {
      Alert.alert(
...
const styles = StyleSheet.create({
  ...
  footer: {
    ...
    borderTopWidth: 1,
    borderTopColor: 'rgba(8, 145, 178, 0.1)',
    shadowColor: COLORS.shadow,
    shadowOffset: { width: 0, height: -4 },
    shadowOpacity: 0.05,
    shadowRadius: 8,
...
export default SeminarEditScreen;